...
class Controller extends ChangeNotifier {
  ...
  checkWord(){
    List<String> guessed = [], remainingCorrect = []; // listas con las letras adivinadas/las que faltan
    String guessedWord = "";

    for(int i = currentRow * 5; i < (currentRow * 5) + 5; i++){
      guessed.add(tilesEntered[i].letter); // añadimos las letras introducidas por el usuario en esta fila
    }

    guessedWord = guessed.join(); // almacenamos la palabra introducida en guessedWord
    remainingCorrect = correctWord.characters.toList(); // guardamos las letras de la palabra correcta
    ...
    if (guessedWord == correctWord){ // el usuario ha acertado la palabra
      ...
      for(int i = 0; i < remainingCorrect.length; i++){
        for(int j = 0; j < 5; j++){
          if(remainingCorrect[i] == tilesEntered[j + (currentRow * 5)].letter){ // si es una letra de la palabra solución

            if(tilesEntered[j + (currentRow * 5)].answerStage != AnswerStage.correct){ // si no es una letra que ya hemos marcado como correcta
              tilesEntered[j + (currentRow * 5)].answerStage = AnswerStage.contains;
            }
            
            final resultKey = keysMap.entries.where((element) => // comprueba si esta letra ya se ha marcado previamente como correcta
            element.key == tilesEntered[j + (currentRow * 5)].letter);

            if(resultKey.single.value != AnswerStage.correct){ // si todavía no se ha marcado como correcta
              keysMap.update(resultKey.single.key, (value) => AnswerStage.contains); // actualizamos
            }
          }
        }
      }
